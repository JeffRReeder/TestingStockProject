...
app.config['SQLALCHEMY_DATABASE_URI'] = (
    os.environ.get('DATABASE_URL', 'postgresql:///swan_stocks'))
...
def list_holding(user_id):
    ...
    stocks = Stock.query.filter( Stock.owner == user_id)



    myStock = Stock.query.filter(Stock.ticker_symbol == 'CSCO').scalar()
    myUser = User.query.filter( User.id == myStock.owner).scalar()
    print('*************************************************')
    ...
    for stock in stocks:
        # ['CSCO', 'AAPL']
        all_ticker_symbols.append(stock.ticker_symbol)
    ...
    return render_template('users/holdings.html', user=users, stock_info=stock_info, API2=dividend, stocks=stocks)
